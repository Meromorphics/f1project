...
data
{
    ...
    // if race i is a race circuit ith entry is 0, if it is a street circuit ith entry is 1
    array[n_races] int<lower=0, upper=1> race_types;
}
...
parameters
{
    ...
    vector[n_drivers] t_r; // driver skills on race circuits, ordered by driver integer identifiers
    vector[n_drivers] t_s; // driver skills on street circuits, ordered by driver integer identifiers
}
...
model
{
    ...
    for (i in 1:n_races)
    {
        ...
        target += get_ln_likelihood(t_d_i + t_c_i + (1-race_type)*t_r[driver_ordering_i] + race_type*t_s[driver_ordering_i]);
    }
...
generated quantities
{
    ...
    for (i in 1:n_races)
    {
        ...
        log_lik[i] = get_ln_likelihood(t_d_i + t_c_i + (1-race_type)*t_r[driver_ordering_i] + race_type*t_s[driver_ordering_i]);
  }
}
